...
ntup_dir = '/eos/user/d/dezhu/HNL/ntuple/20180608_HNLreco/ntuple/'
...
for sig in range(len(V)/3-1):
   tt_V1.Add(ntup_dir + '%s/HNLTreeProducer/tree.root'%V[sig])
for sig in range(len(V)/3-1,2*len(V)/3-1,1):
   tt_V2.Add(ntup_dir + '%s/HNLTreeProducer/tree.root'%V[sig])
for sig in range(2*len(V)/3-1,len(V)-1,1):
   tt_V3.Add(ntup_dir + '%s/HNLTreeProducer/tree.root'%V[sig])
