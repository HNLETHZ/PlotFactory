...
######################################### 
# Declare histograms and canvases
#########################################
h1_den        = ROOT.TH1F('h1_den'       , '', len(bins)-1, bins)
h1_num_muon = ROOT.TH1F('h1_num_muon', '', len(bins)-1, bins)
h1_num_muon_sa = ROOT.TH1F('h1_num_muon_sa', '', len(bins)-1, bins)
h1_num_muon_g = ROOT.TH1F('h1_num_muon_g', '', len(bins)-1, bins)
...
h2_num_muon_sa = ROOT.TH1F('h2_num_muon_sa', '', len(bins)-1, bins)
h2_num_muon_g = ROOT.TH1F('h2_num_muon_g', '', len(bins)-1, bins)
h2_num_dsmuon  = ROOT.TH1F('h2_num_dsmuon' , '', len(bins)-1, bins)
h2_num_dgmuon  = ROOT.TH1F('h2_num_dgmuon' , '', len(bins)-1, bins)

for hh in [h1_den, h1_num_muon, h1_num_dsmuon, h1_num_dgmuon, h2_den, h2_num_muon, h2_num_dsmuon, h2_num_dgmuon, h1_num_muon_sa, h1_num_muon_g, h2_num_muon_sa, h2_num_muon_g]:
...
h1_den       .SetLineColor(ROOT.kBlue) ; h1_den       .SetMarkerColor(ROOT.kBlue) 
h1_num_muon.SetLineColor(ROOT.kBlack) ; h1_num_muon.SetMarkerColor(ROOT.kBlack) 
h1_num_muon_sa.SetLineColor(ROOT.kMagenta+2 ) ; h1_num_muon_sa.SetMarkerColor(ROOT.kMagenta+2 ) 
h1_num_muon_g.SetLineColor(ROOT.kCyan+2 ) ; h1_num_muon_g.SetMarkerColor(ROOT.kCyan+2 ) 
...
h2_den       .SetLineColor(ROOT.kBlue) ; h2_den       .SetMarkerColor(ROOT.kBlue) 
h2_num_muon.SetLineColor(ROOT.kBlack) ; h2_num_muon.SetMarkerColor(ROOT.kBlack) 
h2_num_muon_sa.SetLineColor(ROOT.kMagenta+2 ) ; h2_num_muon.SetMarkerColor(ROOT.kMagenta+2 ) 
h2_num_muon_g.SetLineColor(ROOT.kCyan+2 ) ; h2_num_muon.SetMarkerColor(ROOT.kCyan+2 ) 
h2_num_dsmuon .SetLineColor(ROOT.kRed+2  ) ; h2_num_dsmuon .SetMarkerColor(ROOT.kRed+2  ) 
h2_num_dgmuon .SetLineColor(ROOT.kGreen+2  ) ; h2_num_dgmuon .SetMarkerColor(ROOT.kGreen+2  ) 
...
sel1_num_muon_sa = 'l1_matched_muon_pt > 0 & l1_matched_muon_is_sa==1'
sel1_num_muon_g = 'l1_matched_muon_pt > 0 & l1_matched_muon_is_gl==1'
...
sel2_num_muon_sa = 'l2_matched_muon_pt > 0 & l2_matched_muon_is_sa==1'
sel2_num_muon_g = 'l2_matched_muon_pt > 0 & l2_matched_muon_is_gl==1'
...
tt.Draw('hnl_2d_disp >> h1_num_muon_sa', '&'.join([sel1_den, sel1_num_muon_sa]) )
tt.Draw('hnl_2d_disp >> h1_num_muon_g', '&'.join([sel1_den, sel1_num_muon_g]) )
...
tt.Draw('hnl_2d_disp >> h2_num_muon_sa', '&'.join([sel2_den, sel2_num_muon_sa]) )
tt.Draw('hnl_2d_disp >> h2_num_muon_g', '&'.join([sel2_den, sel2_num_muon_g]) )
...
h1_num_muon_sa.Add(h2_num_muon_sa)
h1_num_muon_g.Add(h2_num_muon_g)
h1_num_dsmuon.Add(h2_num_dsmuon)
h1_num_dgmuon.Add(h2_num_dgmuon)

h1_num_muon.Divide(h1_den)
h1_num_muon_sa.Divide(h1_den)
h1_num_muon_g.Divide(h1_den)
...
pf.showlumi('p_{T} < 10 GeV, |#eta| < 2.4')
leg = ROOT.TLegend(.4,.75,.8,.88)
leg.AddEntry(h1_num_muon, 'slimmedMuons'            ,'EP')
leg.AddEntry(h1_num_muon_sa, 'StadAloneMuons'            ,'EP')
leg.AddEntry(h1_num_muon_g, 'GlobalMuons'            ,'EP')
