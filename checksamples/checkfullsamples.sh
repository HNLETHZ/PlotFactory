file=allsamplefiles.py
numsamp=0
while read -r line; do
   numsamp=$((numsamp+1))	
done < "$file"

echo "number of samples:" $numsamp >> notuseablefull.txt
echo "number of samples:" $numsamp >> useablefull.txt
...
#   echo $signal
   echo -n $enum
   if [ -z "$checkfile" ]; then
      echo $signal >> notuseablefull.txt
   fi
   if [ ! -z "$checkfile" ]; then
       echo $signal >> useablefull.txt
   fi
   if [ $enum -lt 10 ]; then
      echo -n -e "\b"
   fi
